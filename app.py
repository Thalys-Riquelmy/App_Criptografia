...
def predict():
    ...
    encrypted_data, key = encrypt_file(file_path, algorithm)  # Criptografa com a chave gerada
    ...
    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
        zip_file.write(file_path, os.path.basename(file_path))          # Adiciona o arquivo original
        zip_file.write(encrypted_file_path, encrypted_file_name)        # Adiciona o arquivo criptografado
        zip_file.write(key_file_path, key_file_name)                    # Adiciona a chave de descriptografia
...
def encrypt():
    ...
    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
        zip_file.write(encrypted_file_path, encrypted_file_name)  # Adiciona o arquivo criptografado
        zip_file.write(key_file_path, key_file_name)              # Adiciona o arquivo da chave
...
def decrypt():
    ...
    original_extension = os.path.splitext(encrypted_file.filename.replace(".encrypted", ""))[1]
